...
############################################
queries_file = '/export/home/ubuechle/VideoMagnification/sloth/TreadmillGenerated-Sabine-2019-07-28/labeled-data/nameLabelledImages.npy'
queries = np.load(queries_file)

#dt = datetime.now()
#dt = '{}-{}-{}_{}_{}/'.format(dt.year, dt.month, dt.day, dt.hour, dt.minute)
#results_folder = cfg.results_path+'/magnification/magnified/TH%.3f_'%(args.th)+dt
...
for q in queries:
    ...
    frame = int(frame[:-4])
    print(video,frame)
    #print(pos_videos[0],pos_frames[0])
    selection_frames = np.where(np.logical_and(pos_videos==video,pos_frames==frame))[0]
    #selection_frames = [selection_frames]
    #selection_frames = np.where(pos_videos==video)[0]#[:200]
    #np.arange(query-F//2,query+F//2).astype('int')
    frame, coord = pos_frames[selection_frames], pos_coords[selection_frames]
    feat = pos_features[selection_frames]
    F = len(frame)
    #print(frame, frame.shape, coord, coord.shape, feat.shape) 
    ...
    ############################################
    #j, f = 0, frame[0]'
    copyfolder = cfg.results_path+'/magnification/generated_only_labelled/%s/'%(video)
    perimage_folder= cfg.results_path+'/magnification/magnification_pervideo_labelled/%s/'%(video)
    if not os.path.exists(copyfolder): 
        os.makedirs(copyfolder)
    if not os.path.exists(perimage_folder): 
        os.makedirs(perimage_folder+'healthy')
        os.makedirs(perimage_folder+'impaired')
    ...
    for f, fr in enumerate(frame):
        ...
        diff_image12,flow_filtered,X,Y,_ = find_differences_cc(healthy_res,impaired_res,
                                                    magnified_res,Th=0.12,scale=20)
        diff_image20,flow_filtered,X,Y,_ = find_differences_cc(healthy_res,impaired_res,
        ...
        #plt.quiver(X, Y, flow_filtered[:,0],flow_filtered[:,1], width=0.1, 
                   #headwidth=3, color='red',scale_units='width', scale=10,minlength=0.1)
        #plt.close('all')
        _=imsave(copyfolder+'%d.jpg'%(fr),resize(impaired_res, (400,400)))
        #_=imsave(perimage_folder+'healthy/%06d.png'%(fr),healthy_res)
        #_=imsave(perimage_folder+'impaired/%06d.png'%(fr),impaired_res)
        #_=imsave(perimage_folder+'magnified/%06d.png'%(fr),magnified_res)
        #_=imsave(perimage_folder+'difference12/%06d.png'%(fr),diff_image12)
        #_=imsave(perimage_folder+'difference20/%06d.png'%(fr),diff_image20)
        #_=imsave(perimage_folder+'direction/%06d.png'%(fr),diff_image)
        print('Saved',video,frame)
