...
queries = [np.where(pos_videos==v)[0][0]+F//2 for v in np.unique(pos_videos)]
...
for query in tqdm(queries):
    ...
    with open(results_folder+'query_%d.txt'%(query),'w') as txt:
        txt.write('Video: %s\n'%video)
        for f, fr in enumerate(frame):
            txt.write('Frame: %d\n'%(fr))
            for nn in healthy_nn[f]:
                txt.write('%s %d\n'%(h_pos_videos[nn],h_pos_frames[nn]))
    ...
    ############################################
    #j, f = 0, frame[0]
    perimage_folder= cfg.results_path+'/magnification/magnified_perimage/TH%.3f_'%(args.th)+dt+'query%d/'%(query)
    if not os.path.exists(perimage_folder): os.makedirs(perimage_folder)
    ...
    for f, fr in enumerate(frame):
        ...
        diff_image,flow_filtered,X,Y,_ = find_differences_cc(healthy_res,impaired_res,
                                                    magnified_res,Th=args.th,scale=20)
        
        imsave(perimage_folder+video+'_frame%d_healthy.png'%(fr),healthy_res)
        imsave(perimage_folder+video+'_frame%d_impaired.png'%(fr),impaired_res)
        imsave(perimage_folder+video+'_frame%d_magnified.png'%(fr),magnified_res)
        ...
        if f==args.length//2: _=subplot[0][f].set_title(video,fontsize=20,color='black')
        ...
        else: _=subplot[1][f].set_title('%d'%(fr),fontsize=20,color='black')
