...
from model_twoLosses import CaffeNet#######################################################################################################################################################
...
def exp_lr_scheduler(optimizer,iterTrain, lr=0.01, lr_decay_iter=10000):
    ...
    if iterTrain!=0 and iterTrain%lr_decay_iter==0:
        lr = lr*0.1
        print('Learning rate is set to %f'%lr)
...
#adjust learning rate (in case we start in the middle of training)
lr = lr_start
for i in range(iter_old/cfg.train_stepsize):
    optimizer,lr = exp_lr_scheduler(optimizer,(i+1)*cfg.train_stepsize, lr, cfg.train_stepsize)
...
for epoch in range(epoch_old-1,cfg.train_nr_epochs):
    ...
    for i, (data,labels) in enumerate(data_train):
        ...
        #forward pass
        out_permut,out_posture = net(data)
        
        #compute accuracy
        _, predicted = tmax(out_permut.data,1)
        acc = float(sum(labels.cpu().data.numpy()==predicted.cpu().numpy())/float(labels.size(0)))
        
        #backprop1
        optimizer.zero_grad()
        loss_permut = criterion(out_permut, labels)
        loss_posture = -1*criterion2(out_posture[0,:,:],out_posture[1,:,:].data)#"-" because we want to maximize the distance
        loss = alpha*loss_permut+beta*loss_posture
        loss.backward()
        optimizer.step()
        #loss2.backward()
        #optimizer.step()
        ...
        if iter_total%cfg.train_display_freq==0:
            print('%2d/%2d (iter %i), LR %.5f, Loss_permut: % 1.3f, Loss_posture: %1.3f, Accuracy %2.2f%%'\
                %(epoch+1, cfg.train_nr_epochs, iter_total, lr, loss_permut.data.item(),loss_posture.data.item(), 100*acc))
            
            #log the values for tensorboard or save it in a txt file
            if tf_found:
                info = {'loss_permut/train': loss_permut.data.item(), 'loss_posture/train': loss_posture.data.item(), 'accuracy/train': 100*acc, 'learning rate': lr}
                ...
                text_file.write('Epoch %d[%d]) Accuracy %.3f Loss_permut %.3f Loss_posture %.3f LR %.6f\n'%(epoch,i,100*acc,loss_permut.data.item(),loss_posture.data.time(), lr))
    ...
    net.eval()
    accT,lossT_permut,lossT_posture = torch.zeros(len(data_test),dtype=torch.float),torch.zeros(len(data_test),dtype=torch.float),torch.zeros(len(data_test),dtype=torch.float)
    for i,(data,labels) in enumerate(tqdm(data_test)):
        ...
        loss_permut = criterion(out_permut, labels)
        loss_posture = -1*criterion2(out_posture[0,:,:],out_posture[1,:,:].data)
        lossT_permut[i] = float(loss_permut.cpu().data.numpy())
        lossT_posture[i] = float(loss_posture.cpu().data.numpy())
        
    print('Testing Average: Loss1: %.4f, Loss2: %.4f, Accuracy %.1f%%'\
        %(lossT_permut.mean(),lossT_posture.mean(),100*accT.mean()))
    
    #log the values for tensorboard or save it in a txt file
    if tf_found:
        info = {'loss_permut/test': lossT_permut.mean(), 'loss_posture/test': lossT_posture.mean(), 'accuracy/test': 100*accT.mean()}
        for tag, value in info.items():
            logger.scalar_summary(tag, value, iter_total)
    else:
        text_file.write('---Testing: Epoch %d Accuracy %.5f, Loss_permut %.3f, Loss_posture %.3f---\n'%(epoch,100*accT.mean(),lossT_permut.mean(),lossT_posture.mean()))
