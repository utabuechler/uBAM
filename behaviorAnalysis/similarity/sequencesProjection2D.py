#!/usr/bin/env python
# -*- coding: utf-8 -*-

"""
Python 2.7
@author: Biagio Brattoli 
biagio.brattoli@iwr.uni-heidelberg.de
Last Update: 23.8.2018

Use the trained features to plot the data on a 2D space
"""

import os, sys, numpy as np, argparse
from time import time
from tqdm import tqdm, trange

import matplotlib.pyplot as plt
from scipy.io import loadmat
from scipy.misc import imread

from joblib import Parallel, delayed
import multiprocessing
CORES = multiprocessing.cpu_count()

from scipy.spatial.distance import squareform, pdist
from sklearn.preprocessing import normalize
from sklearn.manifold import TSNE
from scipy.stats import kde

from utils import load_table, load_features

try:
    import umap
    IS_UMAP=True
except ImportError:
    IS_UMAP=False
    print('!!UMAP not installed, tSNE will be used!!')

import config_pytorch as cfg

parser = argparse.ArgumentParser()
parser.add_argument("-n", "--n_samples",type=int,default=1000,
                    help="number of samples to plot")
parser.add_argument("-t", "--train_samples",type=int,default=2000,
                    help="number of samples for training the projection")
parser.add_argument("-nn", "--nn_per_image",type=int,default=10,
                    help="nearest neighbors to show on plot")
parser.add_argument("-a", "--algorithm",type=str,default='umap',
                    help="algorithm for dimensionality reduction, ['tsne','umap']")
args = parser.parse_args()

# if __name__ == "__main__":
############################################
# 1. Load Features
############################################
results_fold = cfg.results_path+'/similarity/sequences/'
if not os.path.exists(results_fold): os.makedirs(results_fold)

detections = load_table(cfg.detection_file,asDict=False)
uni_videos = np.unique(detections['videos'].values)
print('Load features...')
features,frames,coords,videos = load_features('lstm',cfg.features_path,uni_videos.tolist())


############################################
# 2. Select samples for reducing computation
############################################
sel = np.linspace(0,features.shape[0]-1,
        min(args.train_samples,features.shape[0])).astype(int)
videos, frames, coords = videos[sel], frames[sel], coords[sel].astype(int)
feat_size = features.shape[1]/cfg.seq_len
features = normalize(features[sel][:,feat_size*(cfg.seq_len-1):].reshape([len(sel),-1]))

############################################
# 3. Dimensionality Reduction
############################################
if args.algorithm=='tsne' or not IS_UMAP:
    clf = TSNE(n_components=2, perplexity=30.0, init='pca')
elif args.algorithm=='umap':
    clf = umap.UMAP(n_neighbors=15,metric='cosine')

t = time()
points = clf.fit_transform(features)
print('Dimensionality Reduction done in %.1fsec'%(time()-t))
np.savez(results_fold+'/sequences_projections2D_%s_train%d'%(args.algorithm,args.train_samples),
                 points=points,videos=videos,
                 frames=frames,coords=coords)

x1, x2 = points[:,0].min()-0.1, points[:,0].max()+0.1
y1, y2 = points[:,1].min()-0.1, points[:,1].max()+0.1

############################################
# 4. Plot 2D Density
############################################
nbins = 100
k = kde.gaussian_kde(points.T)
xi, yi = np.mgrid[x1:x2:nbins*1j, y1:y2:nbins*1j]
zi = k(np.vstack([xi.flatten(), yi.flatten()]))

_=plt.figure(figsize=(14,10))
_=plt.pcolormesh(xi, yi, zi.reshape(xi.shape),cmap=plt.cm.viridis)
_=plt.colorbar()
#plt.contour(xi, yi, zi.reshape(xi.shape) )
_=plt.title('Sequence Density',fontsize=20)
_=plt.xlim([x1,x2])
_=plt.ylim([y1,y2])
_=plt.savefig(results_fold+'/sequences_projection_2D_density_%s_samples%d.png'%(args.algorithm,args.samples),
                    bbox_inches='tight',dpi=75)
# plt.show()

############################################
# 5. Plot 2D Manifold
############################################
def load_image(v,f,c):
    im_path = (cfg.frames_path+v+'/%06d.jpg'%f) 
    if not os.path.exists(im_path): 
        im_path = cfg.frames_path+v+'/%d.jpg'%f
    
    im = imread(im_path)
    return im[c[1]:c[3],c[0]:c[2]]

# Select high density points
distance = squareform(pdist(features,'cosine'))
distance.sort(axis=1)
inv_density = distance[:,:int(0.05*distance.shape[0])].mean(axis=1)
points_sort = inv_density.argsort()

# Non-Maximum suppression
distance = squareform(pdist(points))
distance = distance/distance.max()
peaks_density = [points_sort[0]]
for s in points_sort:
    if distance[s,peaks_density].min()>0.15:
        peaks_density.append(s)

# Plot: one image per peak, show [args.nn_per_image]NN per peak
plot_fold = results_fold+'/high_density_sequences_%s_samples%d/'%(args.algorithm,args.samples)
if not os.path.exists(plot_fold): os.makedirs(plot_fold)

distance = squareform(pdist(features,'cosine'))
F = len(frames[0])
for peak in tqdm(peaks_density,desc='Plot sequences'):
    NN_sequences = distance[:,peak].argsort()
    _=plt.figure(figsize=(30,15))
    _=plt.subplot(1,2,1)
    _=plt.plot(points[:,0],points[:,1],'.b',markersize=5)
    _=plt.plot(points[peak,0],points[peak,1],'.r',markersize=30)
    for i in range(5):
        k = NN_sequences[i]
        for f in range(F):
            im = load_image(videos[k],frames[k][f],coords[k][f])
            _=plt.subplot(5,F*2,1+F+i*2*F+f)
            _=plt.imshow(im)
            _=plt.axis('Off')
    
    _=plt.savefig(plot_fold+'sequence%d_samples%d.png'%(peak,args.samples),bbox_inches='tight', dpi=100)


